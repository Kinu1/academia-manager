...
export function PlanFormPage() {
  ...
  async function handleSubmit(values: PlanFormValues) {
    try {
      ...
      showToast({ title: 'Nao foi possivel salvar o plano.', description: toApiError(error).message, tone: 'danger' })
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          {isEditing ? 'Atualize nome, valor, duracao e status.' : 'Cadastre um plano para vincular alunos.'}
        </p>
      </div>
      ...
      {isEditing && planQuery.isError ? (
        <div className="rounded-lg border border-red-200 bg-white p-6 text-sm font-medium text-red-700 shadow-sm">
          Plano nao encontrado ou indisponivel.
        </div>
      ) : null}
