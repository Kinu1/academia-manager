...
public sealed record StudentResponse(Guid Id, string Name, string Email, string? Phone, StudentStatus Status, Guid? PlanId);
...
public sealed record PlanResponse(Guid Id, string Name, decimal PriceAmount, string PriceCurrency, int DurationInDays, bool IsActive);
...
public sealed record TrainingResponse(Guid Id, Guid StudentId, string Title, string Description, DateTime ScheduledForUtc);
...
public sealed record PaymentResponse(Guid Id, Guid StudentId, decimal Amount, string Currency, DateTime DueDateUtc, DateTime? PaidAtUtc, PaymentStatus Status);
