...
export function TrainingFormPage() {
  ...
  async function handleSubmit(values: TrainingFormValues) {
    ...
    try {
      ...
      navigate(!isEditing && selectedStudentId ? `/students/${selectedStudentId}` : '/trainings')
    } catch (error) {
      showToast({ title: 'Não foi possível salvar o treino.', description: toApiError(error).message, tone: 'danger' })
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          {isEditing ? 'Atualize título, descrição e horário.' : 'Agende um treino para um aluno.'}
        </p>
      </div>
